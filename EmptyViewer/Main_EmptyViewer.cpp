...
Vec3 compute_flat_shading(Vec3 v0, Vec3 v1, Vec3 v2) {
    Vec3 centroid = {
        (v0.x + v1.x + v2.x) / 3.0f,
        (v0.y + v1.y + v2.y) / 3.0f,
        (v0.z + v1.z + v2.z) / 3.0f
    ...
    Vec3 L = normalize({ lightPos.x - centroid.x, lightPos.y - centroid.y, lightPos.z - centroid.z });
    Vec3 V = normalize({ centroid.x, centroid.y, centroid.z });
    Vec3 H = normalize(L + V);

    float NL = std::max(0.0f, dot(N, L));
...
void render_scene()
{
    ...
    for (int i = 0; i < gNumTriangles; ++i)
    {
        ...
        Vec3 color = compute_flat_shading(
            gVertexBuffer[k0],
            gVertexBuffer[k1],
            gVertexBuffer[k2]
        );
